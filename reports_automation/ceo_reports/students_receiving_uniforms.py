"""
Module with custom functions to create students receiving uniforms report.
"""
...
def _get_uniforms_issue_status_summary(df_data, grouping_cols):
    """
    Internal function to get summary of schools' uniform issue statuses at grouping level

    Parameters:
    ----------
    df_data: Pandas DataFrame
        The uniform issued status data to work on
    grouping_cols: list
        The list of columns to group by

    Returns:
    --------
    The summary of schools' uniform issue statuses at grouping level
    """
...
def pre_process_BRC_merge(raw_data:pd.DataFrame):
    """
    Function to process the students receiving uniforms raw data before merging with BRC-CRC mapping data
    ...
    print('Pre Processing before BRC merge called in students receiving uniforms')
...
def get_unranked_elem_report(df_data:pd.DataFrame, grouping_cols:list, agg_dict:dict):
    ...
    # Get uniform issue status wise summary at grouping level
    df_data = _get_uniforms_issue_status_summary(df_data, grouping_cols)
