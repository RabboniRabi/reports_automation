"""
Module with custom functions to create students receiving cycle report.
"""
...
def _get_cycle_issue_status_summary(df_data, grouping_cols):
    """
    Internal function to get summary of schools' cycle issued statuses at grouping level

    Parameters:
    ----------
    df_data: Pandas DataFrame
        The cycle issued status data to work on
    grouping_cols: list
        The list of columns to group by

    Returns:
    --------
    The summary of schools' cycle issues statuses at grouping level
    """
...
def pre_process_BRC_merge(raw_data:pd.DataFrame):
    """
    Function to process the students receiving cycle raw data before merging with BRC-CRC mapping data
    ...
    print('Pre Processing before BRC merge called in students receiving cycle')
...
def get_unranked_elem_report(df_data:pd.DataFrame, grouping_cols:list, agg_dict:dict):
    ...
    # Get cycle boxes issue status wise summary at grouping level
    df_data = _get_cycle_issue_status_summary(df_data, grouping_cols)
