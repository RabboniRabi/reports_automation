"""
Module with custom functions to create students receiving colour pencil report.
"""
...
def _get_colour_pencil_issue_status_summary(df_data, grouping_cols):
    """
    Internal function to get summary of schools' colour pencil issued statuses at grouping level

    Parameters:
    ----------
    df_data: Pandas DataFrame
        The colour pencil issued status data to work on
    grouping_cols: list
        The list of columns to group by

    Returns:
    --------
    The summary of schools' colour pencil issued statuses at grouping level
    """
...
def pre_process_BRC_merge(raw_data:pd.DataFrame):
    """
    Function to process the students receiving colour pencil raw data before merging with BRC-CRC mapping data
    ...
    print('Pre Processing before BRC merge called in students receiving colour pencil')
...
def get_unranked_elem_report(df_data:pd.DataFrame, grouping_cols:list, agg_dict:dict):
    ...
    # Get colour pencil issue status wise summary at grouping level
    df_data = _get_colour_pencil_issue_status_summary(df_data, grouping_cols)
