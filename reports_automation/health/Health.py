...
df1 = pd.read_excel(r'C:\Users\Admin\Downloads\Student-health-checkup-rpt (1).xlsx', sheet_name='Report',skiprows=4)
...
df_e = pd.DataFrame(TV_ED)
df_d = pd.DataFrame(TV_D)

datatoexcel = pd.ExcelWriter(r'C:\Users\Admin\Downloads\health.xlsx')
...
# Part 2 for Format
df5 = pd.read_excel(r'C:\Users\Admin\Downloads\health1.xlsx', sheet_name='Main View')
df5['ERank'] = df5['Edu.Dist']. apply(xlookup, args=(TV_ED['ERank1'], TV_ED['ERank']))

# Vlookup for District Rank

df5['DRank'] = df5['District']. apply(xlookup, args=(TV_D['DRank1'], TV_D['DRank']))

cols = ['Rank', 'DRank', 'ERank']

df5['Overall Rank'] = df5[cols].sum(axis=1)

# Remove unwanted Ranking columns of df5 before printing
df5.drop(columns=df5.columns[-4:-1],
        axis=1,
        inplace=True)


df_5 = pd.DataFrame(df5)
datatoexcel2 = pd.ExcelWriter(r'C:\Users\Admin\Downloads\health_Final2.xlsx')
df_5.to_excel(datatoexcel2, sheet_name='Main View', index=False)
